...
public class ExposicionPerros
{
    ...
    { 	
    	ArrayList<Perro> arrayPerros = darPerros();
    	int tamano = arrayPerros.size();
        Perro temporal = null;

        for (int i = 0; i < tamano; i++) {
            for (int j = 1; j < (tamano - i); j++) {

                if (arrayPerros.get(j - 1).compararPorRaza(arrayPerros.get(j)) == 1  ) {
                    temporal = arrayPerros.get(j-1);
                    arrayPerros.set(j-1, arrayPerros.get(j));
                    arrayPerros.set(j, temporal);
                }
            }
        }
        perros = arrayPerros;
    ...
    {
       	ArrayList<Perro> arrayPerros = darPerros();
        int tamanio = arrayPerros.size();
        Perro temporal = null;
        
        for (int i = 1; i < tamanio; i++) {
            for(int j = i ; j > 0 ; j--){
            	if(arrayPerros.get(j).compararPorNombre(arrayPerros.get(j - 1)) == -1) {
            		temporal = arrayPerros.get(j);
            		arrayPerros.set(j, arrayPerros.get(j-1));
            		arrayPerros.set(j-1, temporal);
                }
            }
        }
        perros = arrayPerros;
    ...
    public void ordenarPorPuntos( )
    {
        ...
        for (int i = 0; i < tamano; i++) {
            for (int j = 1; j < (tamano - i); j++) {
                ...
                if (arrayPerros.get(j - 1).compararPorPuntos(arrayPerros.get(j)) == 1  ) {
                    temporal = arrayPerros.get(j-1);
                    arrayPerros.set(j-1, arrayPerros.get(j));
                    arrayPerros.set(j, temporal);
                }
    ...
    public void ordenarPorEdad( )
    {
        ...
        for (int i = 0; i < tamano - 1; i++)
        {
            ...
            for (int j = i + 1; j < tamano; j++) {
            	if(arrayPerros.get(j).compararPorEdad(arrayPerros.get(posicion)) == -1) {
            		posicion = j;
            	}
            }
                
            Perro menor = arrayPerros.get(posicion);
            arrayPerros.set(posicion, arrayPerros.get(i));
            arrayPerros.set(i, menor);
        }
        perros = arrayPerros;
    }
